...
func (c *Client) GetRepoRulesetByName(ctx context.Context, repoName, name string) (*gogithub.RepositoryRuleset, error) {
	...
	rulesets, _, err := c.restClient.Repositories.GetAllRulesets(ctx, c.org, repoName, &gogithub.RepositoryListRulesetsOptions{
	...
	for _, rs := range rulesets {
		if rs.Name == name {
			full, _, err := c.restClient.Repositories.GetRuleset(ctx, c.org, repoName, rs.GetID(), false)
			if err != nil {
...
func (c *Client) CreateRepoRuleset(ctx context.Context, repoName string, rs gogithub.RepositoryRuleset) (*gogithub.RepositoryRuleset, error) {
	created, _, err := c.restClient.Repositories.CreateRuleset(ctx, c.org, repoName, rs)
	if err != nil {
...
func (c *Client) UpdateRepoRuleset(ctx context.Context, repoName string, rulesetID int64, rs gogithub.RepositoryRuleset) (*gogithub.RepositoryRuleset, error) {
	updated, _, err := c.restClient.Repositories.UpdateRuleset(ctx, c.org, repoName, rulesetID, rs)
	if err != nil {
