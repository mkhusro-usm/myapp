...
// CreateFileChangePR creates a branch, commits a single file change, and opens a pull request.
// Returns the PR's HTML URL. This is the standard workflow for governance rules that apply via PR.
func (c *Client) CreateFileChangePR(ctx context.Context, repoName, baseBranch, filePath, branchPrefix, commitMsg, prTitle, prBody string, content []byte) (string, error) {
